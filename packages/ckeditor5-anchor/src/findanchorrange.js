 ...
 *
 * It can be used e.g. to get the entire range on which the `anchorHref` attribute needs to be changed when having a
 * selection inside a anchor.
 *
 * @param {module:engine/model/position~Position} position The start position.
 * @param {String} value The `anchorHref` attribute value.
...
// Walks forward or backward (depends on the `lookBack` flag), node by node, as long as they have the same `anchorHref` attribute value
...
function _findBound( position, value, lookBack, model ) {
	...
	while ( node && node.getAttribute( 'anchorHref' ) == value ) {
		lastNode = node;
